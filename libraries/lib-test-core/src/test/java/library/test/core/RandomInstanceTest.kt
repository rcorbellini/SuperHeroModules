    ...
    class MakeRandomInstanceTest {
        ...
        fun `Throws NoUsableConstructor error if there is no constructor that could be used`() {
            try {
                ...
                error("library.test.core.makeRandomInstance should throw library.test.core.NoUsableConstructor error")
        ...
        fun `When user expects concrete String length, all Strings have this length`() {
            ...
            repeat(10) {
                ...
                assertEquals(
                    5,
                    makeRandomInstance<List<List<String>>>(config = config)[1][1].length
                )
            }
        }
